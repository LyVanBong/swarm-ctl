...
Công ty bạn (Tên miền: `company.com`) vừa phát triển xong dự án Website tin tức/thương mại. 
...
**Kiến trúc mong muốn:**
- Hệ thống BaaS (Backend-as-a-service): **My Backend API** (chạy tại `api.company.com`).
- Giao diện người dùng: **Frontend Web / ReactJS** (chạy tại `www.company.com`).
...
* **Server 1 (Làm Manager):** IP `192.168.1.10` (Phải có ít nhất 4 Core, 8GB RAM vì nó gánh DB cốt lõi của API).
* **Server 2 (Làm Worker 1):** IP `192.168.1.11` (2 Core, 4GB RAM).
* **Server 3 (Làm Worker 2):** IP `192.168.1.12` (2 Core, 4GB RAM).
...
* Loại: `A` | Host: `api` | Content: `192.168.1.10`
* Loại: `A` | Host: `www` | Content: `192.168.1.10`
...
```bash
swarm-ctl cluster init --master 192.168.1.10 --domain company.com --pass "PassRoot123"
```
...
```bash
swarm-ctl node add --ip 192.168.1.11 --role worker --pass "KhoaBomVuW1_123"
swarm-ctl node add --ip 192.168.1.12 --role worker --pass "KhoaBomVuW2_123"
```
...
## 🟢 BƯỚC 4: TRIỂN KHAI VŨ KHÍ NẶNG (MY BACKEND API)

Giả thiết bạn đã đóng gói mã nguồn và cấu hình của Backend API vào một App Bundle (thư mục `my-api-bundle`). Bạn không cần làm gì ngoài gõ:

```bash
cd /opt/products/
swarm-ctl app deploy ./my-api-bundle --name backend_api
```
...
Còn lõi xử lý API thì được thả cho chạy nhảy trên Cả 3 máy (Tuỳ thằng nào Cổ RAM rảnh rỗi hơn thì ưu tiên ném qua thằng đấy chạy giùm). Người dùng truy cập `https://api.company.com` sẽ được Traefik SSL rải thảm HTTP đưa tin vô mượt mà.

---

## 🟢 BƯỚC 5: NẠP ĐẠN GIAO DIỆN XUẤT TRẬN (FRONTEND WEB)

Giả định Bộ phận lập trình của bạn đã nhồi xong Code HTML CSS vào cái cục tên là `mycompany/ui-react-website:v1.0.0` (Đẩy lên DockerHub công khai hoặc nội bộ).
...
swarm-ctl service add \
  --name frontend-web \
  --image mycompany/ui-react-website:v1.0.0 \
...
Nếu Datacenter số 2 của Máy Worker 2 bị chuột cắn cụt dây mạng (Máy W2 cháy đen/sít off), các khách truy cập vào trang web `www.company.com` lúc 2 giờ sáng vẫn **HIỆN RÕ VÀ KHÔNG BỊ DISCONNECT**. Vì Traefik tại Gateway số 1 Đã nhìn thấy máy 2 chết nên chẻ luồng Mạng rẽ sang Máy Worker số 1.
